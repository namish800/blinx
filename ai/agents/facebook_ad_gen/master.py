...
def continue_to_image_gen(state: AdGeneratorState):
    # We will return a list of `Send` objects
    # Each `Send` object consists of the name of a node in the graph
    # as well as the state to send to that node
    return [Send("image_generator", {"ad_copy": s, "brand_persona": state["brand_persona"],
                                     "campaign_plan": state["campaign_plan"]}) for s in state["ad_copies_intermediate"]]
...
class AdGeneratorAgent:
    def __init__(self):
        ...
        workflow.add_node("image_generator", creator_agent.generate_image)
        ...
        workflow.add_conditional_edges("creator", continue_to_image_gen, ["image_generator"])
    ...
    def run(self, objective, details, brand_persona, config: dict):
        ...
        response: AdGeneratorState = self.graph.invoke(inputs, config)
        return response
...
if __name__ == "__main__":
    ...
    """
    agent_config = {"configurable": {"thread_id": 2004}}
